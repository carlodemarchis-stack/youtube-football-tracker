"""Other Clubs page — football clubs outside the top-5 European leagues on YouTube (FIFA, UEFA, national FAs).
...
- No per-other club detail page — clicking a row opens the YouTube channel
- Powered by its own dedicated daily cron (scripts/daily_other_clubs.py)

Kill-switch: delete this file, the nav entry in app.py, the cron script,
its Railway service, and the OtherClub rows in `channels`. Zero impact elsewhere.
"""
...
if not clubs_o:
    st.info(
        "No other clubs tracked yet. Seed rows with entity_type='OtherClub' "
        "and run `scripts/daily_other_clubs.py` to backfill stats."
    )
    st.stop()
...
_oc_ids = tuple(sorted(p["id"] for p in clubs_o))
_last_by_cid = dict(_cached_last_upload_db(db, _oc_ids, "OtherClub"))
...
_db_id_by_yt = {p.get("youtube_channel_id"): p["id"] for p in clubs_o}
...
_tbl_h = max(len(clubs_o), 1) * 32 + 50  # tightened: rows ~30px, single-row header ~50px
components.html(f"""
<style>
...
log_scale = st.checkbox(
    "Log scale on subscribers (recommended — the biggest dwarf the rest)",
    value=True,
)
...
st.caption(
    ...
    f"({_gss_oc(league='Serie A')} for European football)."
)
