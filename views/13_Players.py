...
_players_updated = _cached_last_fetch(db, "daily_players")
render_page_subtitle(
...
_last_by_cid = dict(_cached_last_upload_db(db, _p_ids, "Player"))
...
(function() {{
  ...
  // Auto-fit iframe to real content so no row is ever clipped.
  function _fit() {{
...
from src.channels import get_season_since as _gss_players
st.caption(
    f"Season window: published since the current-season start date "
    f"({_gss_players(league='Serie A')} for European football)."
)
