...
def _branded_counts(channel_ids: tuple[str, ...]) -> dict[str, int]:
    """Confirmed branded candidates per channel (detection set)."""
    if not channel_ids:
        return {}
    out: list[str] = []
    off = 0
...
st.caption(
    ...
    "(“presented by …”, “powered by …”, “in partnership with …”, plus "
    "ES/IT/DE/FR/PT and CJK/Arabic variants) and extracted the brand. "
    "This surfaces brand deals a channel never formally disclosed, but "
    "it's a heuristic — treat it as an informed guess, not a "
    "confirmation."
)
