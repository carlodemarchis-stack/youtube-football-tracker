"""Federations page — football federations on YouTube (FIFA, UEFA, national FAs).
...
- No per-federation detail page — clicking a row opens the YouTube channel
- Powered by its own dedicated daily cron (scripts/daily_federations.py)

Kill-switch: delete this file, the nav entry in app.py, the cron script,
its Railway service, and the Federation rows in `channels`. Zero impact elsewhere.
"""
...
feds = [c for c in all_channels if c.get("entity_type") == "Federation"]

_feds_updated = _cached_last_fetch(db, "daily_federations")
render_page_subtitle(
    "Football federations on YouTube — FIFA, UEFA, national FAs",
    updated_raw=_feds_updated,
)

if not feds:
    st.info(
        "No federations tracked yet. Seed rows with entity_type='Federation' "
        "and run `scripts/daily_federations.py` to backfill stats."
    )
    st.stop()

# ── Sort by subscribers desc ─────────────────────────────────
feds.sort(key=lambda c: int(c.get("subscriber_count") or 0), reverse=True)

# ── KPIs ─────────────────────────────────────────────────────
_tot_subs = sum(int(c.get("subscriber_count") or 0) for c in feds)
_tot_views = sum(int(c.get("total_views") or 0) for c in feds)
_tot_videos = sum(int(c.get("video_count") or 0) for c in feds)

st.markdown(kpi_row([
    ("Federations tracked",       str(len(feds))),
...
# ── Last-upload lookup (cached) ──────────────────────────────
# Pulls the most-recent published_at per channel from our `videos`
# table; falls through to a YouTube API lookup for federations we
# haven't ingested videos for yet. Both lookups are wrapped in
# @st.cache_data with 1h TTL — used to fire ~50 sync YouTube calls
...
# Map YT channel id → our DB id for the merge
_db_id_by_yt = {p.get("youtube_channel_id"): p["id"] for p in feds}
...
<thead><tr>
  ...
  <th></th>
  <th data-col="2" data-type="str" style="text-align:left">Federation</th>
...
log_scale = st.checkbox(
    "Log scale on subscribers (recommended — FIFA/UEFA dwarf the rest)",
    value=True,
)
...
with col1:
    fig = px.bar(
        ...
        x="Federation", y="Subs",
        color="Federation", color_discrete_map=color_map,
...
_mix_long = _mix.melt(id_vars="Federation", var_name="Format", value_name="Videos")
_mix_long = _mix_long[_mix_long["Videos"] > 0]
if not _mix_long.empty:
    _order = _activity_df["Federation"].tolist()
    fig = px.bar(
        _mix_long, x="Federation", y="Videos", color="Format",
        title="Season video mix by format",
        color_discrete_map={"Long": "#636EFA", "Shorts": "#00CC96", "Live": "#FFA15A"},
        category_orders={"Federation": _order, "Format": ["Long", "Shorts", "Live"]},
    )
    fig.update_layout(
