...
""", unsafe_allow_html=True)
    ...
    # ── Match-day timeline (avg videos per hour on inferred match days) ──
    # A "match day" is any day with at least one Highlights or Match
    # Recap video — a useful proxy when no real match calendar is wired
    # in. For each such day, bucket every video by CET hour and category;
    # divide totals by number of match days to get per-hour averages.
    try:
        from collections import Counter as _Ctr, defaultdict as _DD
        _df_md = df_vids.copy()
        _df_md["_pub_cet"] = _df_md["published_at"].dt.tz_convert(_ZI("Europe/Rome"))
        _df_md["_date"] = _df_md["_pub_cet"].dt.date
        _df_md["_hour"] = _df_md["_pub_cet"].dt.hour
        _df_md["_cat"]  = _df_md["category"].fillna("Other")

        _is_match = (_df_md.groupby("_date")["_cat"]
                     .apply(lambda s: bool(set(s) & {"Highlights", "Match Recap"})))
        _md_dates = set(_is_match[_is_match].index)
        _n_md = len(_md_dates)
        if _n_md >= 5:
            _md_rows = _df_md[_df_md["_date"].isin(_md_dates)]
            # Real-signal categories (drop 'Other'; group rare ones into a tail)
            _SIGNAL_CATS = ["Highlights", "Match Recap", "Matchday",
                            "Press Conference", "Interview", "Goals & Skills",
                            "Celebration & Trophy", "Behind the Scenes",
                            "Throwback"]
            def _bucket(c):
                return c if c in _SIGNAL_CATS else None
            _md_rows = _md_rows.assign(_bucket=_md_rows["_cat"].map(_bucket))
            _signal = _md_rows.dropna(subset=["_bucket"])

            # 24×N matrix: hours × categories (avg per match day)
            _hours = list(range(24))
            _grid = (_signal.groupby(["_hour", "_bucket"]).size()
                     .unstack(fill_value=0)
                     .reindex(index=_hours, columns=_SIGNAL_CATS, fill_value=0)
                     / _n_md)

            st.subheader("📺 Typical match-day timeline")
            st.caption(f"Average videos per hour (CET) on a match day — "
                       f"computed across {_n_md} inferred match days for this "
                       f"club. Stacked by category. 'Other' / promo content is "
                       f"hidden so the match-content rhythm reads cleanly.")

            # Stable per-category palette (matches the Category pies)
            _cat_color = {c: CATEGORY_COLORS.get(c, "#888") for c in _SIGNAL_CATS}
            fig_md = go.Figure()
            for c in _SIGNAL_CATS:
                vals = list(_grid[c].values)
                if sum(vals) <= 0:
                    continue
                fig_md.add_trace(go.Bar(
                    name=c, x=[f"{h:02d}" for h in _hours], y=vals,
                    marker_color=_cat_color.get(c, "#888"),
                    hovertemplate=f"<b>{c}</b><br>%{{x}}:00 CET — "
                                  "%{y:.2f} avg videos/match-day<extra></extra>",
                ))
            fig_md.update_layout(
                barmode="stack", height=320,
                xaxis=dict(title="Hour (CET)", tickmode="array",
                           tickvals=[f"{h:02d}" for h in range(0, 24, 3)],
                           ticktext=[f"{h:02d}" for h in range(0, 24, 3)]),
                yaxis=dict(title="Avg videos per match day",
        ...
        st.caption(f"(match-day timeline unavailable: {_e})")
