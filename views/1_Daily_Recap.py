...
# ── 🚀 Trending now — videos going viral in their first 30 days ───────
# Live (no cron). video_daily_deltas only exists for videos ≤30 days old,
# so by construction this surfaces videos blowing up within their first
# month. A video qualifies if its best single day in the last 3 days
# cleared the viral threshold — robust to single-day YouTube freezes and
# naturally drops a video once it cools off.
VIRAL_THRESHOLD = 250_000


@st.cache_data(ttl=1800, show_spinner=False)
def _load_trending(cids_tuple: tuple, threshold: int, since_date: str) -> list[dict]:
    if not cids_tuple:
        return []
    peak: dict[str, int] = {}
    ...
    for cs in range(0, len(cids), 50):
        chunk = cids[cs:cs + 50]
        # Pull ONLY the days that already cleared the bar (server-side
        # view_delta filter) — keeps this to a handful of rows.
...
_trending = _load_trending(
    _fc_tuple if _fc_tuple else tuple(sorted(filter_cids)),
    VIRAL_THRESHOLD,
