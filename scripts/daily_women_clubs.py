#!/usr/bin/env python3
"""Daily Women Clubs refresh — dedicated cron, isolated from the rest.

For every channel with entity_type='WomenClub':
...
so the Women feature can be killed with zero impact elsewhere.
...
def main() -> int:
    ...
    others = [c for c in channels if c.get("entity_type") == "WomenClub"]
    log(f"Daily Women Clubs refresh — {len(others)} women club(s)")
    if not others:
        log("No Women-Club channels found. Nothing to do.")
        try:
            db.log_fetch(0, 0, "daily_women_clubs", "no others tracked")
        except Exception:
            pass
        return 0
    ...
    # ── 5. Pre-compute the "last upload per channel" map so the Women
    ...
    try:
        ...
        refresh_last_upload(db, "WomenClub", log=log)
    except Exception as e:
    ...
    try:
        status = "daily_women_clubs_partial" if failed else "daily_women_clubs"
    ...
    try:
        ...
        send_run_alert("daily_women_clubs",
                       ok=not failed,
