...
def main() -> int:
    ...
    channels = [c for c in channels if c.get("entity_type") not in ("Player", "Federation", "GoverningBody", "OtherClub", "WomenClub")]
    ...
    channels = db.get_all_channels()
    # Players + Federations have their own dedicated crons — skip them here
    club_channels = [c for c in channels if c.get("entity_type") not in ("League", "Player", "Federation", "GoverningBody", "OtherClub", "WomenClub")]
