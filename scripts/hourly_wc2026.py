...
def main() -> int:
    ...
    try:
        ...
        if total_inserted:
            ...
            # Refresh the WC2026 Latest AI note only when new videos landed
            # (one Anthropic call — no point re-firing on a no-op hour).
            refresh_wc2026_latest_vibe(db, log=log, channels=_fresh_chans)
    except Exception as e:
