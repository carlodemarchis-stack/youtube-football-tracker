...
For every channel with entity_type='Federation':
  ...
  5. Refresh top100_stats for the federation.

Runs at 00:00 UTC (≈ 01:00 CET) — separate service from daily_refresh.py
so the Federations feature can be killed with zero impact elsewhere.
...
def main() -> int:
    ...
    if not federations:
        ...
        try:
            db.log_fetch(0, 0, "daily_federations", "no federations tracked")
        except Exception:
            pass
        return 0
    ...
    for i, ch in enumerate(federations, 1):
        name = ch.get("name", "?")
        ...
        try:
            ...
            ok += 1
            log(f"[{i}/{len(federations)}] {name} — subs={stats.get('subscriber_count', 0):,} "
                f"videos={stats.get('video_count', 0)}")
        except Exception as e:
            failed.append((name, str(e)[:200]))
            log(f"[{i}/{len(federations)}] {name} — ERROR: {e}")

    # ── 4. Snapshot each federation's videos' current stats ──
    # Fetch all video IDs for federation channels and snapshot their counts.
    try:
        fed_cids = [p["id"] for p in federations if p.get("id")]
        if fed_cids:
            ...
            log(f"Snapshotting {len(yt_ids)} federation videos")
            snap_rows: list[dict] = []
    ...
    try:
        status = "daily_federations_partial" if failed else "daily_federations"
