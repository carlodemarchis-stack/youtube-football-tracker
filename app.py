...
def _bootstrap_secrets_toml_from_env() -> None:
    ...
    `[auth]` block — there's no env-var override path. So when the
    five `STREAMLIT_AUTH_*` env vars are set, we materialize a minimal
    secrets.toml at the well-known path so the rest of Streamlit picks
    it up. No-op when:
      - the file already exists (Streamlit Cloud / local dev);
      - the env vars aren't set (running before Phase 2 setup).
    """
    target = os.path.join(".streamlit", "secrets.toml")
    if os.path.exists(target):
        return  # Streamlit Cloud / local-dev path — leave it alone.
    ...
    body = (
        ...
        "# Do not commit the generated file — it's written into the\n"
        "# container's filesystem and stays there for the process's life.\n"
        "[auth]\n"
